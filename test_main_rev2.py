...
def create_raptor_index(documents, chunk_size=1000, chunk_overlap=200):
    """
    RAPTOR 인덱스를 생성합니다.
    ...
    raptor_index = {
        "root": {
            "abstract": generate_root_abstract(abstracts.values(), llm),
            "children": []
        }
    }
    ...
    for group_id, abstract in abstracts.items():
        group_node = {
            "id": group_id,
            "abstract": abstract,
            "chunks": grouped_chunks[group_id],
            "embedding": None  # 임베딩은 나중에 계산
        }
    ...
    # 루트 임베딩
    raptor_index["root"]["embedding"] = embedding_model.embed_query(raptor_index["root"]["abstract"])

    # 그룹 노드 임베딩
    for child in raptor_index["root"]["children"]:
        child["embedding"] = embedding_model.embed_query(child["abstract"])

        # 청크 임베딩
        for chunk in child["chunks"]:
            if "embedding" not in chunk.metadata:
                chunk.metadata["embedding"] = embedding_model.embed_query(chunk.page_content)
